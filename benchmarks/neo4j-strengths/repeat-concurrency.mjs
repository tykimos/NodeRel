...
try {
 for(const concurrency of [8])for(const impl of ['sqlite-optimized','neo4j-bolt']){
  ...
  try{
   ...
   const n=times.length,elapsedMs=performance.now()-started,row={workload,implementation:impl,concurrency,n,elapsedMs,requestsPerSecond:n*1000/elapsedMs,...stats(times),rawMs:times};result.concurrencyRepeat.push(row);console.log(JSON.stringify({...row,rawMs:undefined}));save();
  }finally{await Promise.all(workers.map(w=>w.terminate()));await Promise.all(sessions.map(s=>s.close()));}
 }
 save();console.log('REPEAT COMPLETE');
